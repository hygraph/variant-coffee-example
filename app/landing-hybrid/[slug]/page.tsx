...
import { getDYBackendDecision } from "@/dy/backend-decisions";
...
/**
 * DY experience name pattern: one experience per landing page slug
 * Each experience targets via "Who?" -> Custom Attribute "landing-slug" is "<slug>"
 */
const CAMPAIGN_NAME = "landing-page-hybrid";

interface LandingPageHybridPayload {
  variantId: string;
}

/**
 * Get variant ID from DY backend decision using pageAttributes targeting.
 *
 * Note: Each landing page needs its own DY experience with targeting:
 *   Who? -> Custom Attribute "landing-slug" is "<slug>"
 *   Where? -> Current Page URL contains "landing-hybrid"
 */
async function getDYVariantId(slug: string): Promise<string | undefined> {
  ...
  console.debug("DY backend decision payload:", payload);
...
export async function generateMetadata({ params, searchParams }: Args) {
  ...
  const variantId = await getDYVariantId(slug);
  let landingPage = await getLandingPageBySlug(slug, segment, variantId);
  console.debug("LandingPage Hybrid CMS Query:", {
    slug,
    segment,
    variantId,
    landingPage,
